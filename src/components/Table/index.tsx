...
import { ModalType, OneSelectValueType, TableSignature, TableSignatureValue } from '../../types';
...
const Table = ({ tableData, onActionSuccess }: TableProps) => {
	...
	const [recordsPerPage, setRecordsPerPage] = useState(RECORDS_PER_PAGE_TYPES[1]);
	const checkedRowValue = checkedRowIndex !== null ? tableData.values[checkedRowIndex] : null;
				...
				<View style={[styles.ScrollInnerContainer]}>
					<View style={styles.Row}>
						...
						{tableData.fields.map((field =>
							<View key={field.name} style={[styles.RowCell, styles.RowHeadCell]}>
								<Text style={styles.BoldText} numberOfLines={1}>{field.name}</Text>
								<Text style={styles.BoldText} numberOfLines={1}>{field.type}</Text>
							</View>
						))}
					</View>

					<FlatList
						data={tableData.values.slice((page - 1) * recordsPerPage, page * recordsPerPage)}
						renderItem={(props) =>
							<TableRow
						...
						keyExtractor={(_, index) => `row-${index.toString()}`}
						bounces={true}
			...
			<View style={styles.PagesContainer}>
				...
				<View style={styles.ContainerSelect}>
					<Select
						...
						title=''
						values={[...Array(pagesCount).keys()].map(key => ({ value: key + 1, text: (key + 1).toString() }))}
						select={page}
				...
				<View style={styles.ContainerSelect}>
					<Select
						...
						title=''
						values={RECORDS_PER_PAGE_TYPES.map(number => ({ value: number, text: number.toString() }))}
